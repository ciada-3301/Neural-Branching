...
label_map = {0: "crab", 1: "aquarium_fish", 2: "shark"}
...
fishnet = Model.load("models/fishnet_2.pt")
print("Models loaded successfully!")
parent_layer = 1

# ── Predict function ───────────────────────────────────────────────────────────
def predict_fish(raw_image_norm, threshold=96.0, temperature=3.0):
    ...
    logits      = fishnet.predict([neuron_vals])
...
all_logits = fishnet.predict([all_test_activations])
...
print(f"\n{'':>12}", end="")
for class_name in ["crab", "aquarium_fish", "shark"]:
    print(f"{class_name:>10}", end="")
print("\n" + "-" * 45)

for true_idx, true_class in enumerate(["crab", "aquarium_fish", "shark"]):
...
for i in range(min(15, len(X_test_norm))):
    pred_label, conf = predict_fish(X_test_norm[i])
