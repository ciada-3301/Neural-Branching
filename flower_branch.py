...
# ── Create flowernet model ───────────────────────────────────────────────────────
print("\nCreating flowernet model with anti-overfitting measures...")
flowernet = Model(
    [512, 256, 3],
    activation="relu",
...
print("\nTraining flowernet with manual validation monitoring...")
...
# Prepare data
X_train_tensor = torch.FloatTensor(train_activations).to(flowernet.device)
y_train_tensor = torch.LongTensor(y_train).to(flowernet.device)

X_val_tensor = torch.FloatTensor(val_activations).to(flowernet.device)
y_val_tensor = torch.LongTensor(y_val).to(flowernet.device)
...
optimizer = optim.Adam(flowernet.network.parameters(), lr=0.0003, weight_decay=0.01)
...
for epoch in range(epochs):
    ...
    for batch_X, batch_y in dataloader:
        optimizer.zero_grad()
        outputs = flowernet.network(batch_X)
    ...
    # Validation phase
    flowernet.network.eval()
    with torch.no_grad():
        val_outputs = flowernet.network(X_val_tensor)
    ...
    if val_loss < best_val_loss:
        ...
        best_model_state = flowernet.network.state_dict().copy()
    else:
        patience_counter += 1
    ...
    if patience_counter >= patience:
        ...
        # Restore best model
        flowernet.network.load_state_dict(best_model_state)
        break
...
flowernet.network.eval()
with torch.no_grad():
    train_outputs = flowernet.network(X_train_tensor)
...
# Save model
flowernet.save("models/flowernet_2.pt")
print("\n✓ flowernet saved successfully!")
...
label_names = {0: "sunflower", 1: "poppy", 2: "tulip"}
